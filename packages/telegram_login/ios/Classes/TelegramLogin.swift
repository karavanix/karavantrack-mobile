...
public enum TelegramLogin {
    ...
    private static func performLogin(
        ...
        #if canImport(UIKit)
        let canOpenTelegram = URL(string: "tg://resolve")
            .map { UIApplication.shared.canOpenURL($0) } ?? false
        NSLog("[TG][SDK] performLogin · canOpenTelegram=%@", canOpenTelegram ? "true" : "false")
        if canOpenTelegram {
            if let crossAppURL = try? await fetchCrossAppURL(config: config),
               UIApplication.shared.canOpenURL(crossAppURL) {
                NSLog("[TG][SDK] opening Telegram cross-app URL: %@", crossAppURL.absoluteString)
                await UIApplication.shared.open(crossAppURL)
                return
            }
            NSLog("[TG][SDK] cross-app URL unavailable — falling back to web auth session")
        }
        #endif
    ...
    public static func handle(
        ...
        NSLog("[TG][SDK] handle(url) · %@", url.absoluteString)
        let callback = completion ?? _pendingCompletion
        _pendingCompletion = nil

        guard let callback else {
            NSLog("[TG][SDK] handle(url) · no pending completion — discarding")
            return
        }
        ...
        guard let code = URLComponents(url: url, resolvingAgainstBaseURL: false)?
            ...
            NSLog("[TG][SDK] handle(url) · no 'code' query param in callback URL")
            callback(.failure(TelegramLoginError.noAuthorizationCode))
            return
        }
        NSLog("[TG][SDK] handle(url) · extracted code (%d chars) — exchanging for token", code.count)
    ...
    private static func startWebAuthSession(
        ...
        guard let authURL = buildAuthURL(config: config) else {
            NSLog("[TG][SDK] startWebAuthSession · FAILED to build auth URL")
            completion(.failure(TelegramLoginError.requestFailed("Could not build auth URL")))
            return
        }
        NSLog("[TG][SDK] startWebAuthSession · authURL=%@", authURL.absoluteString)
        ...
        let handleResult: (URL?, Error?) -> Void = { callbackURL, error in
            ...
            if let error {
                ...
                NSLog("[TG][SDK] authSession finished with error · domain=%@ code=%d desc=%@",
                      ns.domain, ns.code, ns.localizedDescription)
                if (error as? ASWebAuthenticationSessionError)?.code == .canceledLogin {
                    NSLog("[TG][SDK] → interpreting as CANCELLED (canceledLogin / code 1)")
            ...
            guard let callbackURL else {
                NSLog("[TG][SDK] authSession returned nil callbackURL")
                completion(.failure(TelegramLoginError.noAuthorizationCode))
                return
            }

            NSLog("[TG][SDK] authSession callbackURL=%@", callbackURL.absoluteString)
            Task { @MainActor in
        ...
        if #available(iOS 17.4, *),
           ...
           let host = components.host {
            ...
            NSLog("[TG][SDK] using HTTPS callback (iOS 17.4+) · host=%@ path=%@", host, path)
            ...
            NSLog("[TG][SDK] using custom-scheme callback · scheme=%@", callbackScheme ?? "nil")
        ...
        NSLog("[TG][SDK] session.start() returned %@", started ? "true" : "false")
    }
    ...
    private static func exchangeCode(
        ...
        NSLog("[TG][SDK] token endpoint · status=%d", statusCode)
        guard statusCode == 200 else {
            NSLog("[TG][SDK] token endpoint error body=%@",
        ...
        if let token = parsed.id_token {
            NSLog("[TG][SDK] token endpoint returned id_token (%d chars)", token.count)
            return token
        }
        NSLog("[TG][SDK] token response has NO id_token · error=%@", parsed.error ?? "nil")
...
private final class PresentationContextProvider: NSObject, ASWebAuthenticationPresentationContextProviding {
    ...
    func presentationAnchor(for session: ASWebAuthenticationSession) -> ASPresentationAnchor {
        ...
        // Prefer the foreground-active scene, then its key window; fall back to
        // any available window so the auth sheet always has a valid anchor.
        let activeScenes = scenes.sorted {
            ($0.activationState == .foregroundActive ? 0 : 1) <
            ($1.activationState == .foregroundActive ? 0 : 1)
        }
        let keyWindow = activeScenes.flatMap { $0.windows }.first { $0.isKeyWindow }
        let anchor = keyWindow
            ?? activeScenes.flatMap { $0.windows }.first
            ?? ASPresentationAnchor()
        NSLog("[TG][SDK] presentationAnchor · scenes=%d keyWindow=%@ fallbackUsed=%@",
              scenes.count,
              keyWindow != nil ? "yes" : "no",
              keyWindow == nil ? "yes" : "no")
        return anchor
    }
}
